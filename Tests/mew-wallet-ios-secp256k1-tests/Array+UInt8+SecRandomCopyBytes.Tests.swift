...
final class Array_UInt8_SecRandomCopyBytes_Tests: XCTestCase {
  ...
  func testSecRandomLength() throws {
    let length: Int = 32
    let randomBytes = try Array<UInt8>.secRandom(length: length)
  ...
  func testSecRandomUniqueness() throws {
    let length: Int = 32
    let randomBytes1 = try Array<UInt8>.secRandom(length: length)
    let randomBytes2 = try Array<UInt8>.secRandom(length: length)
  ...
  func testSecRandomError() {
    XCTAssertThrowsError(try Array<UInt8>.secRandom(length: -1), "Function should throw an error for an invalid length.")
  }
}
